    ...
    <main className="py-10">
      ...
      <form onSubmit={submit} className="mb-6 flex flex-wrap items-center gap-3">
        ...
        <p className="text-xs text-chalk-mid">
          ...
          <button type="button" onClick={() => { setInput("314"); setLeagueId(314); }} className="font-bold text-strike underline">
            314
          </button>{" "}
      ...
      {data && !error && (
        <div className="flex flex-col gap-5">
          <Panel>
            ...
            />
            {projectedLeader && (
              <p className="px-4 py-3 text-sm text-chalk-mid">
                On the model, <b className="text-chalk">{projectedLeader.teamName}</b> has the
                strongest squad for next gameweek at{" "}
                <b className="text-strike">{projectedLeader.projectedXp.toFixed(1)} xP</b>
                ...
                <thead>
                  <tr className="border-b border-line bg-panel-2">
                    {["#", "Team", "Total", "Last GW", "Next GW xP", "Best possible", "Leaking", "Captain", "Top picks"].map((h) => (
                      <th key={h} className="stat px-3 py-3 text-left text-[10px] uppercase tracking-wider text-chalk-dim">
                        {h}
                      </th>
                    ))}
                  </tr>
          ...
          {data.ownership.length > 0 && (
            <Panel className="overflow-hidden">
              ...
              <p className="border-b border-line-soft bg-panel-2 px-4 py-2 text-[12px] text-chalk-mid">
                <b className="text-chalk">Effective ownership</b> counts starters plus captains a
                ...
                <table className="w-full min-w-[720px] border-collapse text-sm">
                  ...
                  <tbody>
                    {data.ownership.slice(0, 20).map((o) => {
                      ...
                      return (
                        <tr key={o.id} className="border-b border-line-soft transition hover:bg-flare-wash">
                          ...
                          <td className="stat px-3 py-2 font-bold text-strike">{o.xp.toFixed(2)}</td>
                          <td className="stat px-3 py-2 text-chalk-mid">{o.ownedPct.toFixed(0)}%</td>
                          <td className="stat px-3 py-2 font-bold">{o.effectivePct.toFixed(0)}%</td>
                          <td className="stat px-3 py-2 text-chalk-mid">{o.captainedBy || "—"}</td>
                          <td className="stat px-3 py-2 text-chalk-dim">{o.globalPct.toFixed(1)}%</td>
                          <td className="px-3 py-2">
  ...
  return (
    <tr className="border-b border-line-soft transition hover:bg-flare-wash">
      <td className="stat px-3 py-2.5 font-bold">
        {e.rank}
        {moved !== 0 && (
          <span className={`ml-1 text-[10px] ${moved > 0 ? "text-surge" : "text-strike"}`}>
      ...
      <td className="px-3 py-2.5">
        ...
        <div className="stat text-[10px] text-chalk-dim">{e.managerName}</div>
      </td>
      <td className="stat px-3 py-2.5 font-bold">{e.total}</td>
      <td className="stat px-3 py-2.5 text-chalk-mid">{e.lastGw}</td>
      <td className="stat px-3 py-2.5">
        {e.hasPicks ? <b className="text-strike">{e.projectedXp.toFixed(1)}</b> : <span className="text-chalk-dim">—</span>}
      </td>
      <td className="stat px-3 py-2.5 text-chalk-mid">
        {e.hasPicks ? e.optimalXp.toFixed(1) : "—"}
      </td>
      <td className="stat px-3 py-2.5">
        {e.leaking > 0.1 ? (
          <span className="font-bold text-flare">−{e.leaking.toFixed(1)}</span>
        ) : (
          <span className="text-surge">optimal</span>
        )}
      </td>
      <td className="px-3 py-2.5">
      ...
      <td className="px-3 py-2.5">
        <div className="stat flex flex-wrap gap-1 text-[10px] text-chalk-dim">
          {e.topPlayers.map((p) => (
            <span key={p.name} className="rounded bg-panel-2 px-1.5 py-0.5">
